...
class data_wrapper:
    def __init__(self, data_dir):
        csv_files = glob.glob(data_dir + "/*prophet_pred.csv")
    ...
    def get_prediction_span(self, zone_name,datetime_start,datetime_end):
        ...
        return self.data[zone_name].loc[(self.data[zone_name]['Date']>=np.datetime64(datetime_start)) & (self.data[zone_name]['Date']<=np.datetime64(datetime_end))]['carbon_per_MWh'].values
